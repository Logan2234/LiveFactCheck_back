...
- [x] **Webhook / notification sur claim** : webhooks **enregistrés par l'utilisateur** (comptes `/v1/users/*` + registre `/v1/webhooks`, `app/models/webhook.py`), déclenchés sur **sa propre session live** (token dans la frame WS `config`) quand un claim d'un **statut configurable** apparaît (plus figé sur "false"). POST best-effort signé HMAC (`X-LFC-Signature`), delivery dans `services/session.py` + `services/webhook.py`. Intégration externe (Slack via relais, overlay OBS…).
